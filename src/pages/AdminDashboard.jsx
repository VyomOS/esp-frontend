...
function VendorApproval({ toast }) {
  ...
  const verify = async id=>{
    try{await adminAPI.verifyVendor(id);toast.success("Vendor verified!");load();}
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
      ...
      <ConfirmModal open={!!confirmVerify} onClose={()=>setConfirmVerify(null)} onConfirm={async()=>{await adminAPI.verifyVendor(confirmVerify.vendor_id);toast.success("Vendor verified!");load();}} title="Verify Vendor" message={`Are you sure you want to verify ${confirmVerify?.organization_name}? They will be notified by email.`} variant="success"/>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{padding:"14px 16px",background:"rgba(248,113,113,0.08)",borderRadius:10,border:"1px solid rgba(248,113,113,0.2)",fontSize:14,color:"var(--text2)"}}>
        Rejecting <strong style={{color:"var(--text)"}}>{vendor?.organization_name}</strong>. The vendor will be notified with your reason and can re-upload documents.
      </div>
      <Textarea label="Rejection Reason *" placeholder="e.g. PAN card image is blurry, please re-upload a clearer copy..." rows={4} value={reason} onChange={e=>setReason(e.target.value)}/>
...
function DocRejectForm({ doc, onReject, onClose }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{fontSize:14,color:"var(--text2)"}}>Rejecting document: <strong style={{color:"var(--text)"}}>{doc?.original_name}</strong></div>
      <Textarea label="Reason (optional)" placeholder="e.g. Image is blurry, please re-upload..." rows={3} value={note} onChange={e=>setNote(e.target.value)}/>
      <div style={{display:"flex",gap:10}}>
        <Btn onClick={onClose} variant="secondary" fullWidth>Cancel</Btn>
...
function UserManagement({ toast }) {
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
      ...
      <ConfirmModal open={!!confirmDeactivate} onClose={()=>setConfirmDeactivate(null)} onConfirm={async()=>{await adminAPI.deactivateUser(confirmDeactivate.id);toast.success("User deactivated");load();}} title="Deactivate User" message={`Are you sure you want to deactivate ${confirmDeactivate?.name}? They will no longer be able to log in.`} variant="danger"/>
    </div>
  );
}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          {/* <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {[{id:"all",label:"👥 Everyone"},{id:"individual",label:"👤 Specific Users"}].map(t=>(
                <button key={t.id} onClick={()=>setTarget(t.id)}
                  style={{flex:1,padding:12,borderRadius:10,border:`2px solid ${target===t.id?"var(--accent)":"var(--border)"}`,background:target===t.id?"rgba(99,132,255,0.1)":"var(--surface)",color:target===t.id?"var(--accent)":"var(--text2)",cursor:"pointer",fontFamily:"Syne",fontWeight:600,fontSize:13,transition:"all 0.2s"}}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          {target==="individual"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <div style={{fontSize:12,color:"var(--text3)"}}>{emails.length} recipient{emails.length!==1?"s":""} added. Press Enter or comma to add each email.</div>
            </div>
          )}
          ...
          {message&&<div style={{padding:"12px 16px",background:"rgba(99,132,255,0.06)",borderRadius:10,border:"1px solid var(--border2)",fontSize:13,color:"var(--text2)",lineHeight:1.6}}><span style={{fontWeight:600,color:"var(--text)",marginRight:8}}>Preview:</span>{message}</div>}
