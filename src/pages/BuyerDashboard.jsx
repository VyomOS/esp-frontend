...
import { Card, StatCard, Btn, Input, Textarea, Modal, SectionHeader, Tabs, Empty, Stars } from "../components/UI";
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {reqs.slice(0,4).map(r=>(
                <div key={r.id} style={{padding:12,background:"var(--bg2)",borderRadius:"var(--radius-sm)",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div><div style={{fontWeight:600,fontSize:14,marginBottom:2}}>{r.title}</div><div style={{fontSize:12,color:"var(--text3)"}}>{r.category} · {r.location}</div></div>
...
function MyRequests({ toast }) {
  ...
  const create = async()=>{
    setSaving(true);
    try{await buyerAPI.createRequest(form);toast.success("Request posted!");setModal(false);setForm({title:"",description:"",category:"",location:"",budget:"",deadline:""});load();}
  ...
  const getAiMatch = async id=>{
    ...
    try{const res=await buyerAPI.aiMatch(id);const m=res.data.matches?.matches||res.data.matches||[];setAiResults(Array.isArray(m)?m:[]);}
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
            ...
            <Card key={r.id}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",borderTop:"1px solid var(--border)",paddingTop:12}}>
                {r.status==="active"&&<>
                  ...
                  <Btn onClick={()=>setConfirmClose(r.id)} variant="ghost" size="sm">Close</Btn>
                </>}
      ...
      <ConfirmModal open={!!confirmDelete} onClose={()=>setConfirmDelete(null)} onConfirm={async()=>{await buyerAPI.deleteRequest(confirmDelete);toast.success("Deleted");load();}} title="Delete Request" message="This will permanently delete the request." variant="danger"/>
      <ConfirmModal open={!!confirmClose} onClose={()=>setConfirmClose(null)} onConfirm={async()=>{await buyerAPI.closeRequest(confirmClose,{close_reason:"Closed by buyer"});toast.success("Request closed");load();}} title="Close Request" message="Are you sure you want to close this request?" variant="danger"/>
      <ConfirmModal open={!!confirmReopen} onClose={()=>setConfirmReopen(null)} onConfirm={async()=>{await buyerAPI.reopenRequest(confirmReopen);toast.success("Request reopened!");load();}} title="Reopen Request" message="This will reopen the request and make it visible to vendors again." variant="success"/>
    </div>
  );
}
...
function RateVendorModal({ open, onClose, toast }) {
  ...
  const save = async()=>{
    ...
    setSaving(true);
    try{await buyerAPI.rateVendor({vendor_id:Number(vendorId),rating,review});toast.success("Rating submitted!");onClose();}
    catch(err){toast.error(err.response?.data?.detail||"Failed");}
    finally{setSaving(false);}
  };
  return (
    <Modal open={open} onClose={onClose} title="Rate a Vendor" width={420}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div><label style={{fontSize:12,fontWeight:600,color:"var(--text2)",display:"block",marginBottom:8,textTransform:"uppercase",letterSpacing:0.5}}>Rating</label><Stars value={rating} onChange={setRating} size={28}/></div>
...
function FindVendors({ toast }) {
  ...
  const load = async()=>{
    ...
    try{
      ...
      else{const r=await vendorAPI.listVendors({search,category,location});setVendors(r.data);}
  ...
  return (
    <div style={{animation:"fadeUp 0.4s ease"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}}>
          {vendors.map((v,i)=>(
            <Card key={v.vendor_id||v.id} glow={i===0&&view==="ranked"} onClick={()=>openVendor(v)}>
              {i===0&&view==="ranked"&&<div style={{marginBottom:12}}><span className="badge verified">🏆 Top Ranked</span></div>}
      ...
      <Modal open={!!selected} onClose={()=>{setSelected(null);setVendorDetail(null);}} title="Vendor Profile" width={580}>
        {detailLoading?<div style={{textAlign:"center",padding:"40px 0"}}><div className="spinner" style={{width:40,height:40,margin:"0 auto 16px"}}/><div style={{color:"var(--text2)"}}>Loading...</div></div>
        :vendorDetail?(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:16}}>
              <div style={{width:56,height:56,background:"linear-gradient(135deg,var(--accent),var(--accent2))",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>🏭</div>
              <div style={{flex:1}}>
                <h3 style={{fontFamily:"Syne",fontWeight:800,fontSize:20,marginBottom:4}}>{vendorDetail.profile?.organization_name||selected?.name}</h3>
              ...
              <div style={{textAlign:"right"}}>
                <div style={{fontFamily:"Syne",fontWeight:800,fontSize:22,color:"var(--warn)"}}>⭐ {vendorDetail.rating?.average_rating?.toFixed(1)||"—"}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {vendorDetail.profile?.phone&&<div style={{padding:"12px 14px",background:"var(--surface)",borderRadius:10,border:"1px solid var(--border)"}}><div style={{fontSize:11,color:"var(--text3)",marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>Phone</div><div style={{fontSize:14,fontWeight:600}}>📞 {vendorDetail.profile.phone}</div></div>}
              {vendorDetail.profile?.website&&<div style={{padding:"12px 14px",background:"var(--surface)",borderRadius:10,border:"1px solid var(--border)"}}><div style={{fontSize:11,color:"var(--text3)",marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>Website</div><a href={vendorDetail.profile.website} target="_blank" rel="noreferrer" style={{fontSize:14,fontWeight:600,color:"var(--accent)",textDecoration:"none"}}>🔗 Visit</a></div>}
            </div>
            {vendorDetail.esg?.length>0&&(
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                  {[{l:"Jobs Created",v:vendorDetail.esg[0].jobs_created,c:"var(--accent)",i:"👷"},{l:"Women Employed",v:vendorDetail.esg[0].women_employed,c:"var(--accent2)",i:"👩"},{l:"Carbon Saved",v:`${vendorDetail.esg[0].carbon_saved}t`,c:"var(--accent3)",i:"🌿"}].map(m=>(
                    <div key={m.l} style={{padding:14,background:"var(--bg)",borderRadius:10,border:`1px solid ${m.c}25`,textAlign:"center"}}>
                      ...
                      <div style={{fontFamily:"Syne",fontWeight:800,fontSize:18,color:m.c}}>{m.v}</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,maxWidth:600}}>
          {notes.map(n=>(
            <div key={n.id} onClick={()=>!n.is_read&&markOne(n.id)}
              style={{padding:"16px 20px",background:n.is_read?"var(--surface)":"rgba(99,132,255,0.08)",border:`1px solid ${n.is_read?"var(--border)":"var(--border2)"}`,borderRadius:"var(--radius-sm)",cursor:n.is_read?"default":"pointer",display:"flex",alignItems:"flex-start",gap:12,transition:"all 0.2s"}}>
