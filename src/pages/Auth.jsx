...
function AuthLayout({ children, title, sub }) {
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex" }}>
      {/* Left panel */}
      <div style={{ flex: 1, display: "none", background: "var(--bg2)", borderRight: "1px solid var(--border)", padding: 48, flexDirection: "column", justifyContent: "space-between" }}
        className="md-flex">
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 64 }}>
            <div style={{ width: 40, height: 40, background: "var(--accent)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Syne", fontWeight: 800, fontSize: 18, color: "white" }}>E</div>
            <div style={{ fontFamily: "Syne", fontWeight: 800, fontSize: 22 }}>ESP Platform</div>
          </div>
          <h1 style={{ fontFamily: "Syne", fontSize: 42, fontWeight: 800, lineHeight: 1.1, marginBottom: 20 }}>
            Ethical<br />Sourcing<br /><span style={{ color: "var(--accent)" }}>Simplified.</span>
          </h1>
          <p style={{ color: "var(--text2)", fontSize: 16, lineHeight: 1.7, maxWidth: 340 }}>
            Connect verified vendors with buyers. Track ESG impact. Automate procurement with AI matching.
          </p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {["🌱 ESG Impact Tracking", "🤖 AI Vendor Matching", "🔒 Verified Vendor Network", "📊 Real-time Analytics"].map(f => (
    ...
    <AuthLayout title="Create account" sub="Join the ESP Platform today">
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <label style={{ fontSize: 13, fontWeight: 500, color: "var(--text2)" }}>I am a</label>
          <div style={{ display: "flex", gap: 8 }}>
            {["buyer", "vendor"].map(r => (
              <button key={r} onClick={() => setForm(p => ({ ...p, role: r }))}
                style={{ flex: 1, padding: "12px", borderRadius: "var(--radius-sm)", border: `2px solid ${form.role === r ? "var(--accent)" : "var(--border)"}`, background: form.role === r ? "rgba(99,132,255,0.1)" : "var(--surface)", color: form.role === r ? "var(--accent)" : "var(--text2)", cursor: "pointer", fontFamily: "Syne", fontWeight: 600, fontSize: 14, textTransform: "capitalize", transition: "all 0.2s" }}>
      ...
      {sent ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Input label="Email address" type="email" placeholder="you@example.com" value={email}
            onChange={e => setEmail(e.target.value)} icon="✉" />
...
export function VerifyEmail() {
  ...
  return (
    <AuthLayout title={status === "success" ? "Email Verified!" : status === "error" ? "Invalid Link" : "Verifying..."}
      sub={status === "success" ? "You can now sign in" : status === "error" ? "This link is invalid or expired" : "Please wait"}>
      <div style={{ textAlign: "center", padding: "32px 0" }}>
